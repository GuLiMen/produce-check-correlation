...
//  println(s"TESTTIME >= '$dayBefore' and TESTTIME < '$today' and IS_VALID = 1")
   ...
   * @param spark
   **/
  def createCheckStageView(spark: SparkSession,startTime : String,endTime :String): Unit = {
    // MESC_TEST_FP_DATA
    spark.read.format("kudu")
      ...
      .where(s"TESTTIME >= '$startTime' and TESTTIME < '$endTime' and IS_VALID = 1")
  ...
  def execLinkQuery(spark: SparkSession, areaView: String): DataFrame = {
    val correlationResult = spark.sql(
      s"""
         ...
         |)b
         |join(
         |    SELECT area,case when COB="COB5" then "COB2" ELSE COB end as cob,eid,device_num,station,sid,main_aa_start,gripperopen_cnt,pathdate FROM (
